...
from pronounce_advanced.models import DailyPractice, DailySubmission
...
class PronounceViewTests(TestCase):
    def setUp(self):
        ...
        self.user.app_access = SimpleNamespace(pronounce_advanced=True)
    ...
    def test_redirect_if_not_logged_in(self):
        response = self.client.get(reverse("pronounce_advanced"))
        login_url = reverse("login") + "?next=" + reverse("pronounce_advanced")
    ...
    def test_post_audio(self, mock_post):
        ...
        response = self.client.post(reverse('pronounce_advanced'), {'word': words[0], 'audio': dummy_audio})
        self.assertEqual(response.status_code, 200)
    ...
    def test_missing_word(self):
        ...
        self.client.login(username='tester', password='complexpass123')
        self._init_session()
        response = self.client.post(reverse('pronounce_advanced'), {'audio': dummy_audio})
    ...
    def test_missing_audio(self):
        self.client.login(username='tester', password='complexpass123')
        words = self._init_session()
        response = self.client.post(reverse('pronounce_advanced'), {'word': words[0]})
        self.assertEqual(response.status_code, 400)
        self.assertIn('Missing audio file', response.content.decode())

    def test_history_requires_login(self):
        response = self.client.get(reverse('pronounce_advanced_history'))
        login_url = reverse('login') + '?next=' + reverse('pronounce_advanced_history')
        self.assertRedirects(response, login_url)
    ...
    def test_history_shows_saved_entry(self, mock_post):
        ...
        words = self._init_session()
        dummy_audio = SimpleUploadedFile('test.wav', b'\x00\x00', content_type='audio/wav')
        self.client.post(reverse('pronounce_advanced'), {'word': words[0], 'audio': dummy_audio})
        resp = self.client.get(reverse('pronounce_advanced_history'))
    ...
    def test_history_parses_string_payload(self):
        from pronounce_advanced.models import PronunciationHistory
        self.client.login(username='tester', password='complexpass123')
    ...
    def test_daily_limit(self, mock_post):
        ...
        dummy_audio = SimpleUploadedFile('test.wav', b'\x00\x00', content_type='audio/wav')
        for i in range(10):
            resp = self.client.post(reverse('pronounce_advanced'), {'word': words[i], 'audio': dummy_audio})
            self.assertEqual(resp.status_code, 200)
        resp = self.client.post(reverse('pronounce_advanced'), {'word': 'extra', 'audio': dummy_audio})
    ...
    def test_progress_persists_across_sessions(self, mock_post):
        ...
        self.client = Client()
        self.client.login(username='tester', password='complexpass123')
        resp = self.client.get(reverse('pronounce_advanced'))
    ...
    def test_unexpected_word(self):
        ...
        words = self._init_session(['a', 'b', 'c'])
        dummy_audio = SimpleUploadedFile('test.wav', b'\x00\x00', content_type='audio/wav')
        resp = self.client.post(reverse('pronounce_advanced'), {'word': 'wrong', 'audio': dummy_audio})
        self.assertEqual(resp.status_code, 400)
